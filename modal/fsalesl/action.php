	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			case 'faktur':
				...
				$status	= 'Faktur';
				$processTitle	= 'Tambah Faktur Lain - Lain '.$nofak;
				// refresh no invoice, faktur & checking no po
				try {
					$qRead = "SELECT * 
								FROM transaksi_faktur_l
								WHERE
									po_tfk=:po_tfk OR
									kode_tfk=:kode_tfk
								LIMIT 1";
                    ...
                    if (is_array($dRead)) {
						array_push($msgBugs, "Nomor Faktur Lain - Lain ".$nomorpo." atau Nomor Faktur Lain- Lain sudah pernah digunakan!");
					}
				...
				if (empty($msgBugs)) {
					...
					try {
						...
						if (is_array($hcek)) {
							array_push($msgBugs, "ID Lain - Lain tidak dapat diproses!");
						}
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
			...
			case 'items':
				...
				$nofak	= $secu->injection($_POST['nomorfaktur']);
				$status	= 'Tagihan';
				$processTitle	= 'Tambah Item Faktur Lain - Lain '.$nofak;
			...
			case 'update':
				...
				// $tglpo	= $secu->injection($_POST['tglpo']);
				// $jatuh	= $secu->injection($_POST['jatuhtempo']);
				$processTitle	= 'Update Faktur Lain - Lain '.$nofak;
				try {
					$edit	= $conn->prepare("UPDATE transaksi_faktur_l SET kode_tfk=:nofak, tgl_tfk=:tglfak, sj_tfk=:nosj, tglsj_tfk=:tglsj, po_tfk=:nopo, updated_at=:catat, updated_by=:admin WHERE id_tfk=:kode");
			...
			case 'delete':
				...
				try {
					$master	= $conn->prepare("SELECT id_psd, jumlah_tfd FROM transaksi_fakturdetail_l WHERE id_tfk=:kode");
				...
				*/
				if (empty($msgBugs)) {
					try {
						$remove	= $conn->prepare("DELETE A, B, C FROM transaksi_faktur_l AS A LEFT JOIN transaksi_fakturdetail_l AS B ON A.id_tfk=B.id_tfk LEFT JOIN pembayaran_faktur AS C ON A.id_tfk=C.id_tfk WHERE A.id_tfk=:kode");
				...
				if (empty($msgBugs)) {
					try {
						$riwayat= $conn->query("INSERT INTO riwayat (kode_riwayat, menu_riwayat, status_riwayat, ket_riwayat, created_at, created_by) VALUES('$kode', 'Faktur Lain - Lain', 'Delete', '', '$catat', '$admin')");
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
				}
