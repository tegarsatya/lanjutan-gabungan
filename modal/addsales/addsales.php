        ...
        <div class="row">
			...
			<table id="example1" class="tabelgetdata">
				...
				<?php
					$status	= 'Active';
					$no		= 1;
					$master	= $conn->prepare("SELECT A.id_psd, A.no_bcode, A.tgl_expired, A.gudang, A.sisa_psd, B.id_pro, B.kode_pro, B.nama_pro, B.berat_pro, C.harga_phg, D.nama_kpr, D.satuan_kpr, E.nama_spr, F.persen_pds FROM produk_stokdetail_b AS A LEFT JOIN produk_b AS B ON A.id_pro=B.id_pro LEFT JOIN produk_harga_b AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk_b AS D ON B.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON B.id_spr=E.id_spr LEFT JOIN produk_diskon_b AS F ON B.id_pro=F.id_pro WHERE $notin AND A.sisa_psd>0 AND C.status_phg=:status AND F.id_out=:mitra GROUP BY A.id_psd");
