...
?>
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">Input Data - Gudang</h6>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
            </button>
        </div>
        <form id="formmenu" action="#" method="post" autocomplete="off">
        <input type="hidden" name="namamodal" id="namamodal" value="gudang" readonly="readonly" />
        <input type="hidden" name="namamenu" value="input" readonly="readonly" />
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-12">
                    <label>Gudang<span class="tx-danger">*</span></label>
                    <input type="text" name="gudang" class="form-control" placeholder="Type here..." required="required" />
...
<?php
		...
		$read	= $conn->prepare("SELECT gudang FROM gudang WHERE id_gudang=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">Update Data - Gudang</h6>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
            </button>
        </div>
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-12">
                    ...
                    <input type="text" name="gudang" class="form-control" value="<?php echo($view['gudang']); ?>" placeholder="Type here..." required="required" />
                	<div id="imgloading"></div>
                </div>
			</div>
        </div>
            ...
            <div class="row">
                <div class="form-group col-md-12">
                    <div class="alert alert-danger alert-dismissible mg-b-0 fade show" role="alert">
                        <strong>Informasi!</strong> Hapus data gudang "<strong><?php echo($view['gudang']) ?></strong>"?
                    </div>
                    <div id="imgloading"></div>
                </div>
			</div>
