	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			case 'faktur':
				...
				$status	= 'Faktur';
				$processTitle	= 'Tambah Faktur Retur'.$nofak;
				// refresh no invoice, faktur & checking no po
				try {
					$qRead = "SELECT * 
								FROM transaksi_faktur_r
								WHERE
									po_tfk=:po_tfk OR
									kode_tfk=:kode_tfk
								LIMIT 1";
				...
				if (empty($msgBugs)) {
					...
					try {
						...
						if (is_array($hcek)) {
							array_push($msgBugs, "ID Retur tidak dapat diproses!");
						}
					} catch (PDOException $e) {
						array_push($msgBugs, $e->getMessage());
					}
			...
			case 'items':
				...
				$nofak	= $secu->injection($_POST['nomorfaktur']);
				$status	= 'Tagihan';
				$processTitle	= 'Tambah Item Faktur Retur'.$nofak;
			...
			case 'update':
				...
				// $tglpo	= $secu->injection($_POST['tglpo']);
				// $jatuh	= $secu->injection($_POST['jatuhtempo']);
				$processTitle	= 'Update Faktur Retur  '.$nofak;
				try {
					$edit	= $conn->prepare("UPDATE transaksi_faktur_r SET kode_tfk=:nofak, tgl_tfk=:tglfak, sj_tfk=:nosj, tglsj_tfk=:tglsj, po_tfk=:nopo, updated_at=:catat, updated_by=:admin WHERE id_tfk=:kode");
			...
			case 'delete':
				...
				try {
					$master	= $conn->prepare("SELECT id_psd, jumlah_tfd FROM transaksi_fakturdetail_r WHERE id_tfk=:kode");
				...
				*/
				if (empty($msgBugs)) {
					try {
						$remove	= $conn->prepare("DELETE A, B, C FROM transaksi_faktur_r AS A LEFT JOIN transaksi_fakturdetail_r AS B ON A.id_tfk=B.id_tfk LEFT JOIN pembayaran_faktur AS C ON A.id_tfk=C.id_tfk WHERE A.id_tfk=:kode");
