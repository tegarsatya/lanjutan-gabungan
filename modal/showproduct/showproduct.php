        ...
        <div class="row">
			...
			<table id="example1" class="tabelgetdata">
				...
				<?php
					$status	= 'Active';
					$no		= 1;
					$master	= $conn->prepare("SELECT A.id_pro, A.kode_pro, A.nama_pro, A.berat_pro, B.harga_phg, C.nama_kpr, C.satuan_kpr, D.nama_spr FROM produk_b AS A LEFT JOIN produk_harga_b AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk_b AS C ON A.id_kpr=C.id_kpr LEFT JOIN satuan_produk AS D ON A.id_spr=D.id_spr WHERE B.status_phg=:status GROUP BY A.id_pro, A.kode_pro, A.nama_pro, A.berat_pro, B.harga_phg, C.nama_kpr, C.satuan_kpr, D.nama_spr ORDER BY A.nama_pro ASC ");
