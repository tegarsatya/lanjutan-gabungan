	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			...
			case 'update':
				...
				try {
					$edit	= $conn->prepare("UPDATE transaksi_faktur_b SET kode_tfk=:nofak, tgl_tfk=:tglfak, sj_tfk=:nosj, tglsj_tfk=:tglsj, po_tfk=:nopo, tglpo_tfk=:tglpo, tgl_limit=:jatuh, updated_at=:catat, updated_by=:admin WHERE id_tfk=:kode");
			...
			case 'delete':
				...
				try {
					$master	= $conn->prepare("SELECT id_psd, jumlah_tfd FROM transaksi_fakturdetail_b WHERE id_tfk=:kode");
				...
				if (empty($msgBugs)) {
					while($hasil = $master->fetch(PDO::FETCH_ASSOC)){
						if (empty($msgBugs)) {
							try {
								$edit	= $conn->prepare("UPDATE produk_stokdetail_b SET keluar_psd=keluar_psd-:jumlah, sisa_psd=sisa_psd+:jumlah WHERE id_psd=:kode");
				...
				*/
				if (empty($msgBugs)) {
					try {
						$remove	= $conn->prepare("DELETE A, B, C FROM transaksi_faktur_b AS A LEFT JOIN transaksi_fakturdetail_b AS B ON A.id_tfk=B.id_tfk LEFT JOIN pembayaran_faktur_b AS C ON A.id_tfk=C.id_tfk WHERE A.id_tfk=:kode");
