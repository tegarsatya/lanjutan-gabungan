...
?>
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">Input Data - Kategori Supplier</h6>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
            </button>
        </div>
        <form id="formtransaksi" action="#" method="post" autocomplete="off">
        <input type="hidden" name="nmenu" id="nmenu" value="ksupplier" readonly="readonly" />
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-12">
                    ...
                    <input type="text" name="nama" class="form-control" placeholder="ex. Industri Farmasi" required="required" />
...
<?php
		...
		$read	= $conn->prepare("SELECT nama_ksp FROM kategori_supplier WHERE id_ksp=:kode");
		$read->bindParam(':kode', $kode, PDO::PARAM_STR);
		$read->execute();
		$view	= $read->fetch(PDO::FETCH_ASSOC);
?>
        <div class="modal-header">
            <h6 class="modal-title" id="exampleModalLabel">Update Data - Kategori Supplier</h6>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true"><i class="fa fa-times-circle"></i></span>
            </button>
        </div>
        ...
        <div class="modal-body">
            <div class="row">
                <div class="form-group col-md-12">
                    <label>Kategori <span class="tx-danger">*</span></label>
                    <input type="text" name="nama" class="form-control" value="<?php echo($view['nama_ksp']); ?>" placeholder="ex. Industri Farmasi" required="required" />
                    <div id="imgloading"></div>
                </div>
			</div>
        </div>
            ...
            <div class="row">
                <div class="form-group col-md-12">
                    <div class="alert alert-danger alert-dismissible mg-b-0 fade show" role="alert">
                        <strong>Informasi!</strong> Hapus data kategori supplier?
                    </div>
                    <div id="imgloading"></div>
                </div>
			</div>
