	...
	if($secu->validadmin($admin, $kunci)==false){
		...
		switch($menu){
			...
			case 'update':
				...
				try {
					$edit	= $conn->prepare("UPDATE transaksi_faktur_p SET kode_tfk=:nofak, tgl_tfk=:tglfak, sj_tfk=:nosj, tglsj_tfk=:tglsj, po_tfk=:nopo, updated_at=:catat, updated_by=:admin WHERE id_tfk=:kode");
