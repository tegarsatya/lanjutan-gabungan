...
<?php
	...
	$read	= $conn->prepare("SELECT A.kode_tfk, A.po_tfk, A.tgl_tfk, B.nama_out FROM transaksi_faktur_p AS A LEFT JOIN outlet AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
...
?>
    <head>
        ...
        <title>Faktur Peminjaman Obat</title>
    ...
    <body>
    	...
    	<table class="tabel">
            ...
            <?php
				...
				$total	= 0;
				$stotal	= 0;
				$master	= $conn->prepare("SELECT A.jumlah_tfd, A.harga_tfd, A.diskon_tfd, B.no_bcode, B.tgl_expired, C.kode_pro, C.nama_pro, C.berat_pro, D.nama_kpr, E.nama_spr FROM transaksi_fakturdetail_p AS A LEFT JOIN produk_stokdetail AS B ON A.id_psd=B.id_psd LEFT JOIN produk AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk AS D ON C.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON C.id_spr=E.id_spr WHERE A.id_tfk=:kode");
