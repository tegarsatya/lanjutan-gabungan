...
<?php
	...
	$read	= $conn->prepare("SELECT A.kode_tfk, A.tgl_tfk, A.tgl_limit, A.po_tfk, TIMESTAMPDIFF(DAY, A.tgl_tfk, A.tgl_limit) AS jarak,B.resmi_out, B.nama_out, B.npwp_out, C.pengiriman_ola FROM transaksi_faktur_b AS A LEFT JOIN outlet_b AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat_b AS C ON B.id_out=C.id_out WHERE A.id_tfk=:kode");
    ...
    <body>
    	...
    	<table class="tabel">
            ...
            <?php
				...
				$stotal	= 0;
				$nomor	= 1;
				$master	= $conn->prepare("SELECT A.jumlah_tfd, A.harga_tfd, A.diskon_tfd, B.no_bcode, B.tgl_expired, C.kode_pro, C.nama_pro, C.berat_pro, D.nama_kpr, E.nama_spr FROM transaksi_fakturdetail_b AS A LEFT JOIN produk_stokdetail_b AS B ON A.id_psd=B.id_psd LEFT JOIN produk_b AS C ON B.id_pro=C.id_pro LEFT JOIN kategori_produk_b AS D ON C.id_kpr=D.id_kpr LEFT JOIN satuan_produk AS E ON C.id_spr=E.id_spr WHERE A.id_tfk=:kode");
