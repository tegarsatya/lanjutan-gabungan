...
?>
    ...
    <body>
    	...
    	<table border="1">
            ...
            <?php
				$nomor	= 1;
				$master	= $conn->prepare("SELECT A.kode_tfk, A.total_tfk, A.status_tfk, A.tgl_tfk, B.po_tsl, C.nama_out FROM transaksi_faktur_b AS A LEFT JOIN transaksi_sales_b AS B ON A.id_tsl=B.id_tsl LEFT JOIN outlet_b AS C ON A.id_out=C.id_out WHERE A.id_tfk!='' $outlet $tgl1 $tgl2 ORDER BY A.tgl_tfk ASC");
				$master->execute();
