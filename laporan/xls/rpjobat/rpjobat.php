...
?>
    ...
    <body>
    	...
    	<table border="1">
            ...
            <?php
				$nomor	= 1;
				$master	= $conn->prepare("SELECT A.jumlah_tfd, B.kode_tfk, B.tgl_tfk, C.no_bcode, C.tgl_expired, D.kode_pro, D.nama_pro, D.berat_pro, E.nama_kpr, F.nama_spr FROM transaksi_fakturdetail_b AS A LEFT JOIN transaksi_faktur_b AS B ON A.id_tfk=B.id_tfk LEFT JOIN produk_stokdetail_b AS C ON A.id_psd=C.id_psd LEFT JOIN produk AS D ON C.id_pro=D.id_pro LEFT JOIN kategori_produk_b AS E ON D.id_kpr=E.id_kpr LEFT JOIN satuan_produk AS F ON D.id_spr=F.id_spr WHERE A.id_tfd!='' $obat $tgl1 $tgl2 ORDER BY B.tgl_tfk DESC");
				$master->execute();
