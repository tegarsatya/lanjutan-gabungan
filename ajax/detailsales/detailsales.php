<?php
	...
	$read	= $conn->prepare("SELECT A.po_tsl, A.tgl_tsl, A.subtot_tsl, A.ppn_tsl, A.total_tsl, B.nama_out, B.npwp_out, C.pengiriman_ola, D.kode_kot FROM transaksi_sales_b AS A LEFT JOIN outlet_b AS B ON A.id_out=B.id_out LEFT JOIN outlet_alamat_b AS C ON B.id_out=C.id_out LEFT JOIN kategori_outlet_b AS D ON B.id_kot=D.id_kot WHERE A.id_tsl=:kode");
	$read->bindParam(':kode', $kode, PDO::PARAM_STR);
	$read->execute();
	$view	= $read->fetch(PDO::FETCH_ASSOC);
	$gabung	= '/FKT/'.$view['kode_kot'].'/'.$data->romawi(date('m')).'/'.date('y');
	$faktur	= $data->transcode($gabung, 'kode_tfk', 'transaksi_faktur_b');

	$subtot	= 0;
	$total	= 0;
	$active	= 'Active';
	$master	= $conn->prepare("SELECT A.id_psd, A.jumlah_tsd, A.harga_tsd, A.diskon_tsd, A.total_tsd, B.id_pro, B.kode_pro, B.nama_pro, B.berat_pro, C.nama_kpr, C.satuan_kpr, D.nama_spr, E.gudang, E.no_bcode, E.tgl_expired FROM transaksi_salesdetail_b AS A LEFT JOIN produk_b AS B ON A.id_pro=B.id_pro LEFT JOIN kategori_produk_b AS C ON B.id_kpr=C.id_kpr LEFT JOIN satuan_produk AS D ON B.id_spr=D.id_spr LEFT JOIN produk_stokdetail_b AS E ON A.id_psd=E.id_psd WHERE A.id_tsl=:kode");
