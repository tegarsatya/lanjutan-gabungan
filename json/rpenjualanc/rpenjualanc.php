<?php
	...
	if($valid==false){
		...
		$tabel	= '';
		$no		= $mulai;
		$jumlah	= $conn->query("SELECT COUNT(A.id_tfd) AS total FROM transaksi_fakturdetail_c AS A LEFT JOIN transaksi_faktur_c AS B ON A.id_tfk=B.id_tfk WHERE A.id_tfd!='' $outlet $produk $tgl1 $tgl2")->fetch(PDO::FETCH_ASSOC);
